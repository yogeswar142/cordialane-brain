...
async function watchSources(sources, includeDotfiles = false, only = [], ignore = []) {
    ...
    return chokidar.watch(sources, {
        ignored: (filename) => {
            if (!includeDotfiles && (0, path_1.basename)(filename).startsWith(".")) {
                return true;
            }
            if (ignore.some(ignore => (0, minimatch_1.minimatch)((0, slash_1.default)(filename), ignore))) {
                return true;
            }
            return only.length > 0 && !only.some(only => (0, minimatch_1.minimatch)((0, slash_1.default)(filename), only));
        },
        ignoreInitial: true,
        awaitWriteFinish: {
